									...
									$sql3=mysqli_query($con,"  UPDATE `tbl_bank_info` SET `balance_amount`=$seller_new_bal WHERE   user_type_id=3 ");
										...
										if($sql3)
										{
											...
											while($rowcart=mysqli_fetch_array($d_cutomer_cart))
											{
												...
												$sql_order_prod=mysqli_query($con,"  INSERT INTO `tbl_customer_order` ( `email`, `stock_product_id`, `delv_adres_id`, `purchase_qty`, `purchase_price`, `order_date`, `delivery_date`, `status`) VALUES ('$mail', $stock_product_id, $delvr_address_id, $purchase_qty, $purchase_price, now(), now()+1, 'UNPAID') ");
